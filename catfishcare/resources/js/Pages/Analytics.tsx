...
const AnalyticsTab = ({ currentData, theme }: AnalyticsTabProps) => {
    ...
    const colors = theme === "light" ? {
        actual: "#0284C7", target: "#64748B", predicted: "#0D9488",
        tooltipBg: "#E2ECE9", tooltipBorder: "rgba(20, 184, 166, 0.2)", tooltipText: "#0F172A", gridColor: "#E2E8F0",
    } : {
        actual: "#38BDF8", target: "#64748B", predicted: "#5DF8D8",
        tooltipBg: "#070D12", tooltipBorder: "rgba(20, 184, 166, 0.15)", tooltipText: "#F8FAFC", gridColor: "rgba(255, 255, 255, 0.05)",
    };

    const tooltipStyle = { backgroundColor: colors.tooltipBg, borderColor: colors.tooltipBorder, color: colors.tooltipText, fontSize: "11px", borderRadius: "6px" };
        ...
        <div className="tab-page analytics-page">
            <div className="analytics-header-row">
                ...
                <div className="ai-model-selector-bar">
                    ...
                    <span>Model AI:</span>
                    <select value={modelType} onChange={(e) => setModelType(e.target.value)} className="model-select-dropdown">
            ...
            <div className="analytics-overview-grid">
                {[
                    { header: "FCR Saat Ini", value: "1.15", desc: "Lebih efisien 4% dibanding target (1.20)", icon: TrendingUp, cls: "text-success", delay: "0.05s" },
                    { header: "Proyeksi Bobot", value: `${(currentData?.Weight || 88.0).toFixed(1)} g`, desc: "Prediksi minggu depan: 132.5 gram", icon: Sparkles, cls: "", delay: "0.1s" },
                    { header: "Estimasi Panen", value: "18 Juni 2026", desc: "Sisa 28 hari menuju bobot ideal (250g)", icon: Calendar, cls: "text-warning", delay: "0.15s" },
                ].map((card, i) => (
                    <div key={i} className="card analytics-overview-card" style={{ animation: "cardFadeIn 0.5s var(--transition-ease) both", animationDelay: card.delay }}>
                        <div className="overview-header">
                            <span>{card.header}</span>
                            <card.icon size={16} color={colors.predicted} />
                        </div>
                        <h3>{card.value}</h3>
                        <p className={`overview-desc ${card.cls}`}>{card.desc}</p>
                    </div>
                ))}
            </div>
            ...
            <div className="card analytics-chart-card">
                <div className="chart-card-header">
                    <div>
                        ...
                        <p>Membandingkan berat aktual vs target ideal vs prediksi LSTM.</p>
                    </div>
                ...
                <div className="analytics-chart-container" style={{ height: "300px", marginTop: "20px" }}>
                    <ResponsiveContainer width="100%" height="100%">
                        <LineChart data={growthProjection} margin={{ top: 10, right: 10, left: -20, bottom: 5 }}>
                            ...
                            <Line type="monotone" dataKey="actualWeight" name="Berat Aktual" stroke={colors.actual} strokeWidth={3} dot={{ r: 4 }} activeDot={{ r: 6 }} animationDuration={500} />
                            <Line type="monotone" dataKey="targetWeight" name="Target Ideal" stroke={colors.target} strokeWidth={2} strokeDasharray="5 5" dot={{ r: 2 }} animationDuration={500} />
                            <Line type="monotone" dataKey="predictedWeight" name="AI Prediksi (LSTM)" stroke={colors.predicted} strokeWidth={2} dot={{ r: 3 }} animationDuration={500} />
                        </LineChart>
                    </ResponsiveContainer>
                </div>
            </div>
                    ...
                    <div className="analytics-chart-container" style={{ height: "200px", marginTop: "15px" }}>
                        <ResponsiveContainer width="100%" height="100%">
                            <AreaChart data={growthProjection} margin={{ top: 5, right: 5, left: -25, bottom: 5 }}>
                                ...
                                <Tooltip contentStyle={tooltipStyle} />
                                <Area type="monotone" dataKey="length" name="Panjang Ikan" stroke={colors.actual} fillOpacity={1} fill="url(#colorLength)" strokeWidth={2} animationDuration={500} />
                            </AreaChart>
                        </ResponsiveContainer>
                    </div>
                </div>
                    ...
                    <div style={{ display: "flex", gap: "10px" }}>
                        ...
                        <div>
                            ...
                            <ul style={{ fontSize: "12px", color: "var(--color-text-muted)", marginTop: "10px", paddingLeft: "20px", lineHeight: "1.6" }}>
                                <li><strong>LSTM</strong>: Memetakan dependensi temporal pertumbuhan berat lele.</li>
                                <li><strong>XGBoost Regressor</strong>: Menganalisis korelasi multivariat pakan, kualitas air, dan FCR.</li>
                            </ul>
                        </div>
                    </div>
                </div>
