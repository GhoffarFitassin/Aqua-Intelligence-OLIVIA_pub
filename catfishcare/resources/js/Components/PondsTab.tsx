...
import { Plus, Search, Filter, HelpCircle, Activity } from 'lucide-react';

const PondsTab = ({ currentData, selectedPondId, setSelectedPondId, setActiveTab }) => {
    ...
    <div className="tab-page ponds-page">
      ...
      <div className="filter-search-container">
        ...
        <div className="filter-tags">
          ...
          >
            Semua ({pondsWithData.length})
          </button>
          <button 
            className={`filter-tag danger ${filterStatus === 'DANGER' ? 'active' : ''}`}
            onClick={() => setFilterStatus('DANGER')}
          >
            🔴 Bahaya ({pondsWithData.filter(p => p.status === 'DANGER').length})
          </button>
          <button 
            className={`filter-tag warning ${filterStatus === 'WARNING' ? 'active' : ''}`}
            onClick={() => setFilterStatus('WARNING')}
          >
            🟡 Waspada ({pondsWithData.filter(p => p.status === 'WARNING').length})
          </button>
          <button 
            className={`filter-tag success ${filterStatus === 'SAFE' ? 'active' : ''}`}
      ...
      <div className="ponds-grid">
        {filteredPonds.map((pond, index) => (
          <div 
            ...
            className={`pond-card status-${pond.status.toLowerCase()} ${pond.id === selectedPondId ? 'active-pond-card' : ''}`}
            style={{ 
          ...
          >
            <div className="pond-card-header">
              <div>
                <h4>
                  {pond.name}
                  {pond.id === selectedPondId && <span className="active-tag-card">Monitor Aktif</span>}
                </h4>
                <span className="pond-type-label">{pond.type}</span>
              </div>
              <span className={`status-dot-badge ${pond.status.toLowerCase()}`}>
                {pond.status}
              </span>
            </div>
            ...
            <div className="pond-card-metrics">
              <div className="pond-sub-stat">
                ...
                <span className="val">{pond.population.toLocaleString('id-ID')} ekor</span>
              </div>
              <div className="pond-sub-stat">
